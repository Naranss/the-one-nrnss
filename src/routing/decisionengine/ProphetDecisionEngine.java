...
public class ProphetDecisionEngine implements RoutingDecisionEngine, EventsDetectionEngine {
	...
	protected DTNHost crNode;
	protected List<DTNHost> destNodes;
	protected List<Set<DTNHost>> nbNodes;
	protected List<Message> msgs;
	protected List<DTNHost> fwNodes;
	protected List<Double> crPreds;
	protected List<Double> fwPreds;
	...
	public ProphetDecisionEngine(Settings s) {
		...
		nbNodes = new ArrayList<>();
		msgs = new ArrayList<>();
		fwNodes = new ArrayList<>();
		crPreds = new ArrayList<>();
		fwPreds = new ArrayList<>();
	...
	public ProphetDecisionEngine(ProphetDecisionEngine de) {
		...
		this.keys = de.keys;
		this.destNodes = de.destNodes;
		this.nbNodes = de.nbNodes;
		this.msgs = de.msgs;
		this.fwNodes = de.fwNodes;
		this.crPreds = de.crPreds;
		this.fwPreds = de.fwPreds;
	}
	...
	public boolean shouldSendMessageToHost(Message m, DTNHost otherHost, DTNHost thisHost) {
		...
		if (m.getTo() == otherHost
				|| de.getPredFor(m.getTo()) > this.getPredFor(m.getTo())) {

			if (crNode == null)
				crNode = thisHost;

			keys.add(SimClock.getTime());
			destNodes.add(m.getTo());
			// getothernb
			List<Connection> cons = thisHost.getConnections();
			Set<DTNHost> setOfnb = new HashSet<>();
			for (Connection con : cons) {
				setOfnb.add(con.getOtherNode(thisHost));
			}
			nbNodes.add(setOfnb);
			msgs.add(m);
			fwNodes.add(otherHost);
			crPreds.add(this.getPredFor(m.getTo()));
			fwPreds.add(de.getPredFor(m.getTo()));
